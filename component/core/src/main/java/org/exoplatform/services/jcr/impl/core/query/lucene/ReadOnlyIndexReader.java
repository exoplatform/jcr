 ...
 * <code>ReadOnlyIndexReader</code> will always show all documents that have not been deleted at the
   ...
   extends FilterIndexReader
{
   ...
   /**
    * The deleted documents as initially read from the IndexReader passed in the constructor of this
    * class.
    */
   ...
   /**
    * Creates a new index reader based on <code>reader</code> at <code>modificationTick</code>.
    * 
    * @param reader
   ...
   /**
    * Increments the reference count on this index reader. The reference count is decremented on
    * {@link #close()}.
    */
   ...
   /**
    * Returns the <code>DocId</code> of the parent of <code>n</code> or {@link DocId#NULL} if
   ...
   /**
    * Unregisters this reader from the shared index reader if the reference count for this reader
    * drops to zero. Specifically, this method does <b>not</b> close the underlying index reader,
   ...
   /**
    * Wraps the underlying <code>TermDocs</code> and filters out documents marked as deleted.
    * 
   ...
   /**
    * Wraps the underlying <code>TermPositions</code> and filters out documents marked as deleted.
    * 
