...
class QueryFormat implements QueryNodeVisitor, QueryConstants
{
   ...
   private QueryFormat(QueryRootNode root, LocationFactory resolver) throws InvalidQueryException
   {
   ...
   /**
    * Creates a SQL <code>String</code> representation of the QueryNode tree argument
    * <code>root</code>.
    * 
    * @param root
    *          the query node tree.
    ...
    *           the query node tree cannot be represented as a SQL <code>String</code>.
    */
   ...
   public Object visit(QueryRootNode node, Object data)
   {
      ...
      try
      {
         ...
         {
            sb.append(" ");
            sb.append(resolver.createJCRName(Constants.NT_BASE).toString());
         }
   ...
   public Object visit(TextsearchQueryNode node, Object data)
   {
      ...
      {
         if (node.getRelativePath().getDepth() > 1 || !node.getReferencesProperty())
         {
   ...
   public Object visit(PathQueryNode node, Object data)
   {
      ...
      try
      {
         if (containsDescendantOrSelf(node))
         {
            ...
            {
               if (steps[i].getNameTest() == null || steps[i].getNameTest().getName().length() > 0)
               {
                  sb.append('/');
               }
   ...
   /**
    * Appends the <code>name</code> to the <code>StringBuffer</code> <code>b</code> using the
    * <code>NamespaceResolver</code> <code>resolver</code>. The <code>name</code> is put in double
    * quotes if the local part of <code>name</code> contains a space character.
    * 
    * @param name
    *          the <code>Name</code> to print.
    * @param resolver
    *          to resolve <code>name</code>.
    * @param b
    *          where to output the <code>name</code>.
   ...
   /**
    * Returns <code>true</code> if <code>path</code> contains exactly one step with a
    * descendant-or-self axis and an explicit name test; returns <code>false</code> otherwise.
    * 
    * @param path
    *          the path node.
    * @return <code>true</code> if <code>path</code> contains exactly one step with a
   ...
   /**
    * Returns <code>true</code> if <code>path</code> contains exactly one location step which matches
    * all node names. That is, matches any children of a given node. That location step must be the
    * last one in the sequence of location steps.
    * 
    * @param path
    *          the path node.
