...
/**
 * Extends a <code>MultiReader</code> with support for cached <code>TermDocs</code> on
   ...
   implements HierarchyResolver, MultiIndexReader
{
   ...
   /**
    * Reference count. Every time close is called refCount is decremented. If refCount drops to zero
   ...
   /**
    * Returns the DocId of the parent of <code>n</code> or {@link DocId#NULL} if <code>n</code> does
   ...
   /**
    * Increments the reference count of this reader. Each call to this method must later be
   ...
   /**
    * Decrements the reference count and closes the underlying readers if this reader is not in use
    * anymore.
    * 
    * @throws IOException
