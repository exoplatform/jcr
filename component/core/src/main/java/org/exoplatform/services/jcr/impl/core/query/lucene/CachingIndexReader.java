...
class CachingIndexReader extends FilterIndexReader
{
    ...
    */
   private static final Log log = ExoLogger.getLogger(CachingIndexReader.class);
   ...
   /**
    * Cache of nodes parent relation. If an entry in the array is not null, that means the node with
    * the document number = array-index has the node with <code>DocId</code> as parent.
    */
   ...
   /**
    * Creates a new <code>CachingIndexReader</code> based on <code>delegatee</code>
    * 
    * @param delegatee
    *          the base <code>IndexReader</code>.
    * @param cache
    *          a document number cache, or <code>null</code> if not available to this reader.
    */
