...
/**
 * Implements an <code>IndexReader</code>, that will close when all connected clients are
 * disconnected AND the <code>SharedIndexReader</code>s <code>close()</code> method itself has been
 * called.
 */
   ...
   extends FilterIndexReader
{
   ...
   /**
    * Set to <code>true</code> if this index reader should be closed, when all connected clients are
    * disconnected.
    */
   private boolean closeRequested = false;

   /**
    * Map of all registered clients to this shared index reader. The Map is rather used as a Set,
    * because each value is the same Object as its associated key.
    */
   private final Map clients = new IdentityHashMap();

   /**
    * Creates a new <code>SharedIndexReader</code> which is based on <code>in</code>.
    * 
    * @param in
   ...
   /**
    * Registeres <code>client</code> with this reader. As long as clients are registered, this shared
    * reader will not release resources on {@link #close()} and will not actually close but only
   ...
   /**
    * Closes this index if no client is registered, otherwise this reader is marked to close when the
