...
public class WorkspaceTraversalResult implements QueryResult
{
    ...
    *          the namespace resolver of the session.
    */
   public WorkspaceTraversalResult(Session session, InternalQName[] properties, LocationFactory resolver)
   {
   ...
   public String[] getColumnNames() throws RepositoryException
   {
      try
      {
         ...
         {
            propNames[i] = resolver.createJCRName(properties[i]).getAsString();
         }
         return propNames;
      }
   ...
   {
      return new RowIteratorImpl(getNodeIterator(), properties, resolver, session.getValueFactory());
   }
   ...
   /**
    * Returns a {@link ScoreNodeIterator} that traverses the workspace in document order.
    * 
    * @return iterator that returns nodes in document order.
    * @throws RepositoryException
    *           if an error occurs while creating the iterator.
    */
   ...
   private class TraversingNodeIterator implements ScoreNodeIterator, Offset
   {
      ...
      /**
       * The current <code>Node</code>, which acts as the starting point for the traversal.
       */
      private final Node currentNode;

      /**
       * The chain of iterators which includes the iterators of the children of the current node.
       */
      private IteratorChain selfAndChildren;

      /**
       * Offset of this iterator to calculate the value in {@link #getPosition()}.
       */
       ...
       * 
       * @param start
       *          the node from where to start the traversal.
       * @param offset
       *          the offset to use to calculate the position.
       */
      ...
      /**
       * Returns the offset value for this iterator. The offset is the current position plus 1.
       * 
