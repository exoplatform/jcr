...
/**
 * <code>DynamicPooledExecutor</code> implements an executor, which dynamically adjusts its maximum
 * number of threads according to the number of available processors returned by
...
public class DynamicPooledExecutor
{
   ...
   /**
    * Executes the given command. This method will block if all threads in the pool are busy and
    * return only when the command has been accepted. Care must be taken, that no deadlock occurs
    * when multiple commands are scheduled for execution. In general commands should not depend on
   ...
   /**
    * Executes a set of commands and waits until all commands have been executed. The results of the
    * commands are returned in the same order as the commands.
    * 
    * @param commands
   ...
   public static class Result
   {
      ...
      /**
       * @return the result object or <code>null</code> if an exception was thrown.
       */
      public Object get()
      {
         return object;
