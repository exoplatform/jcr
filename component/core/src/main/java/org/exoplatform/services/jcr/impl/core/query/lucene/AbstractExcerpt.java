...
/**
 * <code>AbstractExcerpt</code> implements base functionality for an excerpt provider.
 */
   ...
   implements HighlightingExcerptProvider
{
    ...
    * {@inheritDoc}
    */
   public String getExcerpt(String nodeId, int maxFragments, int maxFragmentSize) throws IOException
   {
      ...
      try
      {
         ...
         {
            log.debug("No TermPositionVector on Fulltext field, using " + SimpleExcerptProvider.class.getName());
            SimpleExcerptProvider exProvider = new SimpleExcerptProvider();
            exProvider.init(query, index);
            return exProvider.getExcerpt(nodeId, maxFragments, maxFragmentSize);
         }
   ...
   /**
    * Creates an excerpt for the given <code>text</code> using token offset information provided by
    * <code>tpv</code>.
    * 
    * @param tpv
   ...
   /**
    * Makes sure the {@link #query} is rewritten. If the query is already rewritten, this method
    * returns immediately.
    * 
    * @param reader
    *          an optional index reader, if none is passed this method will retrieve one from the
