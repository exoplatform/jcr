...
CREATE PROCEDURE JCR_NEXT_VAL (@nameSeq VARCHAR(120))
AS
BEGIN
SET NOCOUNT ON
DECLARE @seq INTEGER
UPDATE JCR_SEQ SET nextVal = nextVal + 1 WHERE name = @nameSeq
SELECT @seq = nextVal FROM JCR_SEQ where name=@nameSeq
